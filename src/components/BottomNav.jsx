...
export default function BottomNav({ badgeCount }) {
  const base = 'flex flex-col items-center gap-0.5 flex-1 py-2 text-[11px] font-medium text-gray-400 transition-colors'
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex pb-safe z-50">
      ...
      <NavLink to="/geral" className={({ isActive }) => `${base} ${isActive ? active : ''}`}>
        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
